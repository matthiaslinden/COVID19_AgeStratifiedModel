...
#    mdeaths[datetime.date(2020,4,26)] = {
 #       "male" : [   0,   0,   0,   4,  32, 132, 380, 900,1368, 369,   3],
  #      "female":[   0,   0,   0,   0,   8,  41, 124, 404,1185, 620,  29]}
    ...
    mdeaths[datetime.date(2021,6,29)] = {
        "male" : [   5,   6,  53, 157, 476,2167,5730,12001,20544, 6555,  0],
        "female":[  10,   5,  29,  77, 227, 883,2504, 6781,19850,12604,  0]}

    # Juli 2021
    mdeaths[datetime.date(2021,7,6)] = {
        "male" : [   4,   6,  53, 159, 479,2167,5763,12038,20593, 6557,  0],
        "female":[  10,   5,  29,  79, 229, 891,2517, 6808,19879,12624,  0]}

    mdeaths[datetime.date(2021,7,13)] = {
        "male" : [   4,   6,  53, 160, 481,2198,5790,12062,20616, 6568,  0],
        "female":[  10,   5,  29,  81, 229, 895,2528, 6831,19915,12645,  0]}

    mdeaths[datetime.date(2021,7,27)] = {   # published 2021-07-29
        "male" : [   4,   6,  53, 163, 485,2230,5821,12117,20663, 6575,  0],
        "female":[  10,   5,  31,  81, 232, 898,2545, 6862,19958,12674,  0]}
    
    # August 2021
    mdeaths[datetime.date(2021,8,3)] = {    # Datenstand 03.08.2021
        "male" : [   4,   6,  54, 163, 487,2241,5833,12134,20673, 6585,  0],
        "female":[  10,   5,  31,  80, 234, 905,2552, 6873,19978,12687,  0]}

    mdeaths[datetime.date(2021,8,10)] = {    # Datenstand 
        "male" : [   3,   6,  54, 164, 488,2245,5847,12154,20700, 6593,  0],
        "female":[  10,   5,  31,  81, 235, 909,2557, 6885,19994,12703,  0]}

    mdeaths[datetime.date(2021,8,17)] = {    # Datenstand 
        "male" : [   3,   6,  54, 165, 481,2247,5855,12172,20714, 6599,  0],
        "female":[  10,   6,  31,  81, 236, 911,2563, 6893,20008,12715,  0]}

    mdeaths[datetime.date(2021,8,24)] = {    # Datenstand 24.08.2021
        "male" : [   3,   6,  54, 166, 492,2254,5872,12191,20736, 6604,  0],
        "female":[  10,   5,  32,  81, 237, 915,2571, 6903,20028,12723,  0]}

    mdeaths[datetime.date(2021,8,31)] = {    # Datenstand 31.08.2021
        "male" : [   4,   6,  55, 166, 494,2266,5885,12204,20766, 6615,  0],
        "female":[  11,   5,  33,  82, 242, 916,2583, 6923,20066,12738,  0]}

    mdeaths[datetime.date(2021,9,8)] = {
        "male" : [   4,   7,  56, 171, 502,2284,5905,12231,20815, 6629,  0],
        "female":[  11,   5,  33,  84, 247, 921,2592, 6953,20097,12761,  0]}

    mdeaths[datetime.date(2021,9,15)] = {
        "male" : [   4,   6,  57, 172, 515,2300,5928,12268,20873, 6651,  0],
        "female":[  11,   6,  33,  84, 256, 930,2608, 6980,20148,12803,  0]}

    mdeaths[datetime.date(2021,9,22)] = {
        "male" : [   4,   6,  60, 174, 532,2319,5959,12317,20929, 6675,  0],
        "female":[  11,   6,  36,  87, 258, 936,2624, 7009,20202,12840,  0]}

    mdeaths[datetime.date(2021,9,29)] = {
        "male" : [   4,   6,  62, 182, 541,2342,5994,12375,21022, 6704,  0],
        "female":[  11,   6,  36,  88, 262, 950,2645, 7039,20283,12876,  0]}

    mdeaths[datetime.date(2021,10,6)] = {
        "male" : [   4,   6,  63, 185, 550,2359,6031,12422,21107, 6727,  0],
        "female":[  11,   6,  36,  92, 268, 958,2662, 7076,20338,12913,  0]}

    mdeaths[datetime.date(2021,10,13)] = {
        "male" : [   4,   6,  63, 194, 562,2375,6071,12468,21204, 6757,  0],
        "female":[  11,   7,  36,  95, 272, 968,2681, 7117,20403,12950,  0]}

    mdeaths[datetime.date(2021,10,20)] = {
        "male" : [   4,   6,  63, 197, 570,2394,6002,12510,21279, 6791,  0],
        "female":[  11,   8,  36,  96, 275, 977,2707, 7145,20500,12991,  0]}

    mdeaths[datetime.date(2021,10,27)] = {
        "male" : [   5,   6,  65, 204, 576,2419,6139,12585,21410, 6837,  0],
        "female":[  11,   7,  36,  99, 277, 986,2722, 7191,20589,13048,  0]}

    mdeaths[datetime.date(2021,11,3)] = {
        "male" : [   6,   9,  67, 208, 582,2441,6190,12646,21547, 6894,  0],
        "female":[  11,   8,  36,  99, 286, 993,2748, 7237,20730,13143,  0]}

    mdeaths[datetime.date(2021,11,10)] = {
        "male" : [   6,   9,  68, 210, 583,2468,6250,12755,21763, 6963,  0],
        "female":[  12,   8,  37, 102, 294,1011,2782, 7308,20908,13270,  0]}

    mdeaths[datetime.date(2021,11,17)] = {
        "male" : [   6,   9,  69, 216, 608,2516,6343,12896,22092, 7076,  0],
        "female":[  12,   8,  37, 105, 300,1029,2821, 7393,21163,13428,  0]}

    mdeaths[datetime.date(2021,11,24)] = {
        "male" : [   7,  10,  69, 226, 622,2553,6440,13079,22445, 7195,  0],
        "female":[  13,   9,  41, 109, 312,1053,2872, 7505,21455,13596,  0]}

    mdeaths[datetime.date(2021,12,1)] = {
        "male" : [   6,  10,  69, 234, 636,2622,6589,13347,22906, 7314,  0],
        "female":[  13,  10,  41, 111, 325,1080,2951, 7669,21880,13813,  0]}

    mdeaths[datetime.date(2021,12,8)] = {
        "male" : [   7,  10,  69, 236, 666,2714,6768,13649,23451, 7459,  0],
        "female":[  14,  10,  43, 116, 332,1111,3015, 7831,22321,14057,  0]}

    mdeaths[datetime.date(2021,12,15)] = {
        "male" : [   7,   9,  71, 247, 694,2819,6983,13983,24039, 7661,  0],
        "female":[  15,  11,  44, 121, 340,1150,3120, 8019,22825,14348,  0]}

    mdeaths[datetime.date(2021,12,22)] = {
        "male" : [   6,   9,  73, 256, 721,2948,7209,14385,24582, 7816,  0],
        "female":[  15,  12,  46, 127, 347,1200,3248, 8228,23318,14599,  0]}

    mdeaths[datetime.date(2021,12,29)] = {
        "male" : [   6,  10,  75, 268, 740,3035,7392,14667,25006, 7911,  0],
        "female":[  16,  15,  46, 129, 358,1225,3327, 8376,23665,14776,  0]}

    mdeaths[datetime.date(2022,1,5)] = {
        "male" : [   6,  11,  77, 276, 764,3100,7563,14902,25341, 8007,  0],
        "female":[  17,  13,  48, 131, 366,1256,3405, 8405,24005,14948,  0]}

    mdeaths[datetime.date(2022,1,12)] = {
        "male" : [   8,  11,  78, 282, 791,3183,7755,15125,25715, 8102,  0],
        "female":[  16,  13,  49, 136, 378,1287,3489, 8636,24345,15152,  0]}

    mdeaths[datetime.date(2022,1,19)] = {
        "male" : [  11,  12,  82, 287, 802,3241,7899,15299,25971, 8178,  0],
        "female":[  16,  14,  51, 141, 383,1311,3551, 8770,24601,15275,  0]}

    mdeaths[datetime.date(2022,1,26)] = {
        "male" : [  12,  13,  82, 290, 816,3296,7989,15441,26156, 8228,  0],
        "female":[  16,  16,  51, 141, 392,1340,3609, 8857,24803,15386,  0]}

    mdeaths[datetime.date(2022,2,2)] = {
        "male" : [  15,  15,  82, 299, 834,3340,8084,15561,26378, 8307,  0],
        "female":[  17,  16,  55, 141, 398,1358,3644, 8930,24996,15499,  0]}

    mdeaths[datetime.date(2022,2,9)] = {
        "male" : [  17,  14,  84, 300, 850,3379,8172,15699,26593, 8372,  0],
        "female":[  18,  16,  55, 144, 405,1379,3684, 9006,25188,15639,  0]}

    mdeaths[datetime.date(2022,2,16)] = {
        "male" : [  16,  13,  85, 305, 857,3409,8256,15839,26883, 8474,  0],
        "female":[  19,  17,  58, 145, 409,1399,3738, 9110,25446,15784,  0]}

    mdeaths[datetime.date(2022,2,23)] = {
        "male" : [  16,  15,  86, 306, 863,3456,8337,15996,27215, 8596,  0],
        "female":[  20,  18,  57, 151, 411,1416,3777, 9234,25742,15992,  0]}

    mdeaths[datetime.date(2022,3,2)] = {
        "male" : [  18,  14,  85, 308, 876,3480,8420,16149,27513, 8715,  0],
        "female":[  19,  19,  57, 152, 418,1435,3822, 9329,26032,16170,  0]}

    mdeaths[datetime.date(2022,3,9)] = {
        "male" : [  19,  14,  86, 313, 884,3511,8501,16331,27873, 8875,  0],
        "female":[  20,  18,  60, 156, 424,1453,3872, 9421,26330,16389,  0]}

    mdeaths[datetime.date(2022,3,16)] = {
        "male" : [  19,  14,  87, 317, 891,3548,8584,16507,28183, 8995,  0],
        "female":[  19,  18,  61, 160, 428,1466,3914, 9523,26611,16584,  0]}

    mdeaths[datetime.date(2022,3,23)] = {
        "male" : [  16,  14,  88, 319, 895,3569,8662,16653,28512, 9119,  0],
        "female":[  23,  19,  62, 163, 430,1477,3956, 9615,26916,16799,  0]}

    mdeaths[datetime.date(2022,3,30)] = {
        "male" : [  17,  16,  88, 321, 902,3610,8746,16838,28875, 9264,  0],
        "female":[  21,  20,  64, 164, 433,1493,4006, 9724,27257,17028,  0]}

    mdeaths[datetime.date(2022,4,6)] = {
        "male" : [  17,  16,  88, 323, 912,3636,8847,17034,29260, 9400,  0],
        "female":[  21,  24,  62, 167, 438,1515,4065, 9838,27548,17270,  0]}

    mdeaths[datetime.date(2022,4,13)] = {
        "male" : [  17,  16,  90, 328, 921,3663,8934,17226,29636, 9551,  0],
        "female":[  22,  25,  63, 171, 441,1539,4108, 9965,27931,17501,  0]}

    mdeaths[datetime.date(2022,4,20)] = {
        "male" : [  18,  16,  91, 330, 927,3682,8992,17329,29864, 9618,  0],
        "female":[  21,  23,  63, 172, 444,1550,4139,10041,28116,17639,  0]}

    mdeaths[datetime.date(2022,4,27)] = {
        "male" : [  18,  17,  91, 334, 935,3714,9083,17511,30219, 9758,  0],
        "female":[  21,  23,  63, 174, 453,1571,4177,10144,28441,17850,  0]}


    mdeaths[datetime.date(2022,6,1)] = {
        "male" : [  19,  16,  97, 338, 966,3822,9339,17981,31228,10175,  0],
        "female":[  23,  27,  66, 176, 471,1620,4310,10453,29269,18454,  0]}

    mdeaths[datetime.date(2022,6,8)] = {
        "male" : [  19,  16,  98, 338, 965,3831,9371,18050,31320,10209,  0],
        "female":[  24,  28,  68, 177, 473,1624,4322,10488,29356,18514,  0]}

    mdeaths[datetime.date(2022,6,15)] = {
        "male" : [  20,  18,  99, 339, 971,3842,9405,18108,31428,10248,  0],
        "female":[  26,  26,  67, 179, 477,1626,4341,10532,29447,18585,  0]}

    mdeaths[datetime.date(2022,6,22)] = {
        "male" : [  22,  17,  99, 340, 972,3849,9442,18165,31543,10276,  0],
        "female":[  26,  27,  68, 180, 479,1637,4359,10572,29519,18628,  0]}

    mdeaths[datetime.date(2022,6,29)] = {
        "male" : [  21,  17, 100, 341, 974,3857,9467,18240,31671,10316,  0],
        "female":[  27,  28,  68, 181, 481,1646,4374,10613,29653,18702,  0]}
