...
#        print(nsex,nclinical_def,year)
 #       print("index",isex,iclinical_def,groups)
            ...
            try:
                ...
                if qrows != None:
                    ...
                    for row in rows:
                        ...
                        counts = {}
                        for k,v in zip(["total"]+[x for x in range(81)]+["unknown"],row["Values"]["string"]):
                            if v != None:
                                counts[k] = int(v)
    ...
    def AsyncCrawl(self):
        for y in [2020,2021]:
            self.AsyncCrawlYear(y)
            
        self.WriteToFile()
        
    def AsyncCrawlYear(self,year):
        """ Crawl all LKs using the full set of filters """
        t1 = time.time()
        filters = self.PrepareFilters(year)
...
#        for lk in self.lks.GetLandkreisIDs():
    ...
    def WriteToFile(self):
        ...
        with open("lks_%d.pickle"%ts,"bw+") as f:
